...
int main(void)
{
  ...
  while (1)
  {
      ...
      if (HAL_GPIO_ReadPin(nivel_GPIO_Port, nivel_Pin) == GPIO_PIN_RESET &&
      ...
      if (HAL_GPIO_ReadPin(encher_GPIO_Port, encher_Pin) == GPIO_PIN_RESET && (current_time - last_esvaziar_press_time) > DEBOUNCE_TIME) {
      ...
      if (HAL_GPIO_ReadPin(esvaziar_GPIO_Port, esvaziar_Pin) == GPIO_PIN_RESET && (current_time - last_esvaziar_press_time) > DEBOUNCE_TIME) {
          ...
          if (enchendo) {
              if (freqAtual > freqAlvo) {
                  freqAtual -= 500;  // vai diminuindo até o alvo
              ...
              if (freqAtual < FREQ_VAZIO) {
                  freqAtual += 500;  // vai subindo até "vazio"
